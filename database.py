...
class Database:
    ...
    def init_db(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS models (
                ...
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                strategy_name TEXT DEFAULT 'None',
                custom_prompt TEXT,
        ...
        # Upgrade database schema - add new columns if they don't exist
    ...
    def _upgrade_schema(self, cursor):
        """Upgrade database schema by adding new columns"""
        # Check and add indicators_config column to models table
        cursor.execute("PRAGMA table_info(models)")
        columns = [column[1] for column in cursor.fetchall()]

        if 'indicators_config' not in columns:
            cursor.execute('ALTER TABLE models ADD COLUMN indicators_config TEXT')

        # Add live trading columns
        if 'live_trading_enabled' not in columns:
            cursor.execute('ALTER TABLE models ADD COLUMN live_trading_enabled INTEGER DEFAULT 0')

        if 'live_exchange' not in columns:
            cursor.execute('ALTER TABLE models ADD COLUMN live_exchange TEXT')

        if 'live_symbol' not in columns:
            cursor.execute('ALTER TABLE models ADD COLUMN live_symbol TEXT DEFAULT "BTC/USDT"')

        # Add trading interval column (per-model trading frequency)
        if 'trading_interval_minutes' not in columns:
            cursor.execute('ALTER TABLE models ADD COLUMN trading_interval_minutes INTEGER DEFAULT 60')
    ...
    def get_portfolio(self, model_id: int, current_prices: Dict = None) -> Dict:
        ...
        # Calculate realized P&L (sum of all trade P&L minus fees)
        cursor.execute('''
            SELECT
                COALESCE(SUM(pnl), 0) - COALESCE(SUM(fee), 0) as total_pnl
            FROM trades
            WHERE model_id = ?
        ''', (model_id,))
    ...
    def add_model(self, name: str, provider_id: int, model_name: str, initial_capital: float = 10000,
                  strategy_name: str = "None", custom_prompt: str = None, indicators_config: str = None,
                  live_trading_enabled: bool = False, live_exchange: str = None, live_symbol: str = "BTC/USDT",
                  trading_interval_minutes: int = 60) -> int:
        """Add new trading model"""
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO models (name, provider_id, model_name, initial_capital, strategy_name, custom_prompt,
                               indicators_config, live_trading_enabled, live_exchange, live_symbol, trading_interval_minutes)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (name, provider_id, model_name, initial_capital, strategy_name, custom_prompt, indicators_config,
              1 if live_trading_enabled else 0, live_exchange, live_symbol, trading_interval_minutes))
